...
async def _cleanup_links():
    ...
    await motor_db.classes.delete_many({"class_id": {"$regex": "^sched-test-"}})
    await motor_db.login.delete_many({"user_id": {"$regex": "^sched-test-"}})
    await motor_db.absence_alerts.delete_many({"class_id": {"$regex": "^sched-test-"}})
    await motor_db.parent_reminder_log.delete_many({"class_id": {"$regex": "^sched-test-"}})
    await motor_db.parent_links.delete_many({"student_user_id": {"$regex": "^sched-test-"}})
...
async def _make_class(class_id: str, teacher_id: str, student_ids: list, *, time_str: str, family_alerts: bool):
    now = datetime.now(timezone.utc)
    await motor_db.classes.insert_one({
        "class_id": class_id,
        "teacher_id": teacher_id,
        "org_id": "",
        "days": [_DAY_ABBRS[now.weekday()]],
        "time": time_str,
...
async def test_check_absences_sends_alert_without_crashing(monkeypatch):
    """Regression for the h/m NameError (segment 4.1) that killed every run."""
    class_start = datetime.now(timezone.utc) - timedelta(minutes=45)
    class_id = f"sched-test-{secrets.token_hex(4)}"
    teacher_id = f"sched-test-{secrets.token_hex(4)}"
    student_id = f"sched-test-{secrets.token_hex(4)}"
    student_email = f"sched-test-{secrets.token_hex(4)}@example.com"
    ...
    await _make_class(class_id, teacher_id, [student_id], time_str=class_start.strftime("%H:%M"), family_alerts=True)

    sent = []
    monkeypatch.setattr("app.email_service.send_email", lambda *a, **k: sent.append((a, k)))

    await check_absences()  # must not raise NameError
...
async def test_check_absences_one_bad_class_does_not_abort_others(monkeypatch):
    """A single class raising mid-loop must not stop later classes from being evaluated."""
    class_start = datetime.now(timezone.utc) - timedelta(minutes=45)
    good_class_id = f"sched-test-{secrets.token_hex(4)}"
    bad_class_id = f"sched-test-{secrets.token_hex(4)}"
    teacher_id = f"sched-test-{secrets.token_hex(4)}"
    student_id = f"sched-test-{secrets.token_hex(4)}"
    student_email = f"sched-test-{secrets.token_hex(4)}@example.com"
...
async def test_send_class_reminders_ignores_family_alerts_flag(monkeypatch):
    ...
    class_start = datetime.now(timezone.utc) + timedelta(minutes=10)
    class_id = f"sched-test-{secrets.token_hex(4)}"
    teacher_id = f"sched-test-{secrets.token_hex(4)}"
    student_id = f"sched-test-{secrets.token_hex(4)}"
    parent_id = f"sched-test-{secrets.token_hex(4)}"
    ...
    await _make_class(class_id, teacher_id, [student_id], time_str=class_start.strftime("%H:%M"), family_alerts=False)

    fake = _FakeTwilio()
    monkeypatch.setattr("twilio.rest.Client", lambda *a, **kw: fake)

    await send_class_reminders()
...
async def test_send_class_reminders_dedup_is_atomic(monkeypatch):
    ...
    await send_class_reminders()
    await send_class_reminders()  # simulates a second worker/tick hitting the same window
