...
async function renderSettings() {
    ...
    // Both settings below exist only to control the auto-detect overlay, which is
    // the Premium "Email meeting detection" feature. With no entitlement there is
    // nothing here to configure, so the gear that opens this page is hidden too
    // (see renderDashboard). If a free-tier setting is ever added, show the gear
    // again and gate just these rows.
    ...
    app.innerHTML = `
        ...
        <div class="settings-body">
            ${!isPremium ? '<div class="setting-desc" style="padding:14px 16px">No settings available on your plan.</div>' : `
...
async function renderDashboard() {
    ...
    document.getElementById('app').innerHTML = `
        <div class="header">
            ...
            ${isPremium ? `<button class="gear-btn" id="settings-btn" aria-label="Settings">
